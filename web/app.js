  ...
  dashPlayground = async function dashPlaygroundV3() {
    ...
    $('#ihp-form')?.addEventListener('submit',async e=>{
      ...
      try{
        ...
        const combined=[...searchResults.map(x=>({url:x.url,title:x.title,meta:(x.description||'Web search result'),kind:'SEARCH'})),...pages.map(x=>({url:x.url,meta:'Depth '+fmt(x.depth)+' · '+fmt(x.links_found)+' links · '+(x.elapsed_ms==null?'—':fmt(Math.round(x.elapsed_ms))+' ms'),kind:x.error?'ERROR':'CRAWLED',error:x.error}))];
        ...
        '<div class="research-stats"><div><small>PAGES</small><b>'+fmt(summary.pages)+'</b></div><div><small>SUCCESS</small><b>'+fmt(summary.successful)+'</b></div><div><small>DISCOVERED</small><b>'+fmt(summary.discovered_urls)+'</b></div><div><small>LINKS</small><b>'+fmt(summary.links_found)+'</b></div><div><small>EVIDENCE</small><b>'+fmt(summary.evidence_successful||0)+'</b></div><div><small>TIME</small><b>'+fmt(summary.duration_ms)+'<em> ms</em></b></div></div>'+
        '<div class="result-tabs"><button class="active" data-result-tab="sources">Sources</button><button data-result-tab="evidence">Evidence</button><button data-result-tab="trace">Run trace</button><button data-result-tab="raw">Raw data</button></div><div id="result-pane"></div>';
        const evidenceCards=evidence.map((x,i)=>'<article><span>'+(i+1)+'</span><div><b>'+esc(x.title||x.url)+'</b><small>'+esc((x.text||x.description||x.error||'').slice(0,420))+'</small><small>'+esc(x.url)+'</small></div><em class="'+(x.error?'bad':'good')+'">'+(x.error?'FAILED':'EXTRACTED')+'</em></article>').join('');
        const panes={sources:'<div class="source-list">'+(sourceCards||'<p>No source pages returned.</p>')+'</div>',evidence:'<div class="source-list evidence-list">'+(evidenceCards||'<p>No extracted evidence for this run.</p>')+'</div>',trace:'<div class="trace-list">'+trace+'</div>',raw:'<pre class="research-raw"><code>'+esc(JSON.stringify(data,null,2))+'</code></pre>'};
